...
export class MailQueue {
    ...
    async addWelcomeEmailJob(to: string, name: string) {
        try {
            ...
            const jobPromise = this.emailQueue.add('welcome', {
                to,
                name,
            }, {
                attempts: 3,
            ...
            const timeoutPromise = new Promise((_, reject) =>
                setTimeout(() => reject(new Error('Queue addition timed out (Redis likely unreachable)')), 5000)
            );
            ...
            this.logger.error(`Failed to add welcome email job to queue: ${error.message}`);
            throw error;
        }
    }
}